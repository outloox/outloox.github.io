...
const contactFormSchema = (dictionary: Dictionary['contact']) => z.object({
  name: z.string().min(2, { message: dictionary.error_name_message }),
  email: z.string().email({ message: dictionary.error_email_message }),
...
export function ContactForm({ dictionary, lang }: ContactFormProps) {
  ...
  const formAction = async (prevState: any, formData: FormData) => {
    ...
    if (!validatedFields.success) {
        return { success: false, message: dictionary.error_general_message };
    }
    ...
    if (telegramSuccess) {
      ...
      return { success: false, message: "The message could not be delivered." };
    }
  };
  
  const [state, dispatchFormAction] = useActionState(formAction, null);
